...
- **Sourcing = free RSS + per-user BYOK APIs** — RSS from CoinDesk, CoinTelegraph, Decrypt, The Block is public/keyless. CryptoPanic (`?currencies=`) and CryptoCompare news are free-tier APIs where **each user supplies their own API key** (entered in the Account → API Keys → News Aggregates tab). No web scraping; no social-media sources.
- **News API keys follow the M10 BYOK model** — KMS-encrypted in the `UserData` table under `BYOK#cryptopanic` / `BYOK#cryptocompare`, using a new `category: 'news'` field in the provider registry. These are **not** project `sst.Secret`s; there is no project-side API key and no project metering for these two APIs. Quota is per-user (the user's own free-tier account).
...
//   ProviderId     += 'cryptopanic' | 'cryptocompare'
//   ByokProvider   += category: 'social' | 'apify' | 'news'   // new field on the provider registry
//   PROVIDERS      += cryptopanic: { id: 'cryptopanic', name: 'CryptoPanic', category: 'news', enabled: true }
//                    cryptocompare: { id: 'cryptocompare', name: 'CryptoCompare', category: 'news', enabled: true }
// UserData row: BYOK#cryptopanic / BYOK#cryptocompare  (same KMS-encrypted shape as all other BYOK providers)
...
- Add `cryptopanic` and `cryptocompare` BYOK providers to `packages/core/src/providers.ts`: extend `ProviderId`, add `category: 'news'` field to `ByokProvider`, register both providers.
- New validation client modules `packages/core/src/lib/cryptopanic.ts` and `packages/core/src/lib/cryptocompare.ts` (probe-key functions used by the Account UI on save).
- New **"News Aggregates"** tab/segment in the Account → API Keys UI, alongside the existing "Per-source keys" and "Apify" segments. Users enter and validate their CryptoPanic / CryptoCompare API keys here; keys are shown as •••• last4, removable, with a background-polling opt-in (same pattern as other BYOK providers).
...
- CryptoPanic and CryptoCompare keys are resolved **per-user** via the BYOK resolver (`getByokKey(userId, provider)` / `getPollAssignments('cryptopanic')` / `getPollAssignments('cryptocompare')`) — the same pattern used by the social pollers. There is no project-wide key for either API.
- `packages/core/src/lib/cryptopanic.ts` and `packages/core/src/lib/cryptocompare.ts` gain their news-fetch wrappers on top of the Phase 1 validation clients (mirror `lib/twitter.ts`).
...
- Couples with M5 for any optional Bedrock classification (sentiment, tier-gated). **CryptoPanic and CryptoCompare are pure BYOK** — no project metering for these two APIs (users pay against their own free-tier quota, mirroring Reddit/Apify BYOK). RSS is keyless and has no quota impact.
...
- **Free-API rate limits / cost** — CryptoPanic and CryptoCompare quota is per-user (BYOK); no project metering for these two APIs. RSS uses HTTP conditional GET + 5-minute cadence — confirm RSS feed update frequency before Phase 2. Confirm each user's current free-tier API quotas (CryptoPanic/CryptoCompare) when onboarding.
