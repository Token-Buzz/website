...
- **Apify is a per-user BYOK provider** (`apify`) in the registry (`providers.ts` / `providerMeta.ts`), KMS-encrypted in `UserData` under `BYOK#apify` exactly like the other keys, validated on entry via `GET /v2/users/me`.
- **Ingestion-mode setting** lives at `UserData` `SETTINGS#ingestion` (`db/ingestion-mode.ts`) holding `{ default, overrides }`. The pure resolver `resolveIngestionMode(settings, source)` (`sources/ingestion-mode.ts`, unit-tested) returns the per-source override if set, else the global default.
- **Second adapter implementation, not a new abstraction:** `sources/apify-adapter.ts` provides `APIFY_ADAPTERS` (one per source, `byokProvider: 'apify'`); `getAdapter(id, mode)` selects the Apify implementation when the resolved mode is `apify`, else the direct adapter. Entitlement (`allowedSources`) stays keyed by source identity, so mode changes only the *implementation*, never the tier gate. Each source maps to a default public Apify actor via `lib/apify-actors.ts`, whose pure, defensive normalizers map actor dataset rows → `RawTweet` → `enrichRawTweet` → `putTweet` (reusing the existing `Tweets` table, like Reddit/Telegram/Discord/Farcaster — no per-source table).
- **Dispatch wiring:** `/api/query` loads the user's ingestion settings once and dispatches each source via `getAdapter(source, modeFor(source))`; an Apify-mode source resolves the shared `BYOK#apify` token, and an `ApifyApiError` 401/403 marks that token invalid. The monitor poller is mode-aware too: `getMonitorAssignments` carries `mode` on each `MonitorTask`, the per-source/keyless paths skip sources resolved to `apify`, and a new Apify-holder path emits `mode: 'apify'` tasks — direct and Apify paths are mutually exclusive per `(user, source)`.
- **UI:** the API Keys tab gains a segmented `[ Per-source keys ] [ Apify ]` control (sets the global default + switches the view), an Apify token panel, per-source override toggles (Default / Per-source / Apify) with a live "effective mode" readout, and an Apify-pricing cost note. Apify is excluded from the per-source provider tabs (`getPerSourceProviderMeta`).
- **Sync vs async — deviation from the spec above (documented like prior phases):** both manual search **and** automated monitoring run the Apify actor synchronously via `run-sync-get-dataset-items` (a tight `timeoutSecs` cap; a 408 timeout yields a partial/empty result rather than a hard failure). This fits the existing **synchronous** `SourceAdapter.search/since` interface and the poller with no new infra, and is fully testable offline. The async **Apify webhook → ingest Lambda** path described above is **deferred as a follow-up optimization** — it is not required for correctness (run-sync already covers polling) and would add an API Gateway + Lambda + Apify-webhook-registration subsystem that can't be exercised in the offline harness.
- **Actor ids** are sensible public-actor defaults (e.g. `apidojo~tweet-scraper`, `trudax~reddit-scraper`); normalizers are defensive so a differing actor output shape degrades gracefully rather than throwing.
