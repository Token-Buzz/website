...
/**
 * GET /api/tokens/:symbol/news-alert
 * Returns { available, enabled } for the per-token news alert.
 *   available = the token has newsKeywords configured (needed for news matching).
...
export async function GET(
  ...
  try {
    ...
    const available = Boolean(
      profile?.newsKeywords && profile.newsKeywords.length > 0,
    );

    const entries = await listWatchlistEntries(userId);
    const enabled = entries.some(
      (e) => e.symbol.toUpperCase() === sym && e.newsAlerts === true,
    );

    return Response.json({ available, enabled });
  } catch (err) {
    console.error("[GET /api/tokens/:symbol/news-alert] failed:", err);
    return Response.json({ error: "Internal server error" }, { status: 500 });
  }
}

/**
 * PUT /api/tokens/:symbol/news-alert
 * Body: { enabled: boolean }
 *
 * enabled=true:  ensure a watchlist entry exists for the symbol (create one
 *                with a suggested query if none), then set newsAlerts=true.
 * enabled=false: set newsAlerts=false on every matching entry (no-op if none).
 */
export async function PUT(
  ...
  try {
    ...
    if (enabled) {
      ...
      await setWatchlistAlertPrefs(userId, entryId, { newsAlerts: true });
    } else {
      for (const entry of matching) {
        await setWatchlistAlertPrefs(userId, entry.entryId, { newsAlerts: false });
      }
    }

    return Response.json({ enabled });
  } catch (err) {
    console.error("[PUT /api/tokens/:symbol/news-alert] failed:", err);
    return Response.json({ error: "Internal server error" }, { status: 500 });
  }
}
