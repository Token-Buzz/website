...
export function SpikeCards() {
  ...
  useEffect(() => {
    ...
    fetchSpikes();
    const interval = setInterval(fetchSpikes, 30000); // Refresh every 30s
    return () => clearInterval(interval);
  }, []);

  // Generate random sparkline data for each spike
  const generateSparkline = (seed: number) => {
    const points = [];
    for (let i = 0; i < 20; i++) {
      points.push(Math.sin(i * 0.2 + seed) * 50 + 50 + Math.random() * 20);
    }
    return points;
  };
            ...
            >
              ...
              {isLive && (
                ...
                >
                  ...
                  />
                  <span style={{ fontSize: "9px", fontWeight: 600, color: "var(--data-amber)", textTransform: "uppercase" }}>
                    Live
                  </span>
                </div>
              )}
