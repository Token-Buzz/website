...
export function PulseSection() {
  ...
  return (
    <div style={{ marginBottom: "32px" }}>
      <div style={{ background: "var(--data-bg)", border: "1px solid rgba(255, 107, 44, 0.2)", borderRadius: "var(--r-2)", padding: "24px" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            {(["15m", "1h", "4h"] as const).map((r) => (
              <button
                key={r}
                onClick={() => setRange(r)}
                style={{ padding: "6px 12px", fontSize: "12px", fontWeight: 500, border: "1px solid", borderColor: range === r ? "var(--data-amber)" : "rgba(255, 107, 44, 0.2)", background: range === r ? "rgba(255, 107, 44, 0.1)" : "transparent", color: range === r ? "var(--data-amber)" : "var(--data-fg)", borderRadius: "var(--r-1)", cursor: "pointer", transition: "all 200ms" }}
              >
                {r}
              </button>
            ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: "20px" }}>
          ...
          <div>
            <div style={{ fontSize: "11px", color: "var(--data-fg)", opacity: 0.6, marginBottom: "6px" }}>Change from avg</div>
            <div style={{ fontSize: "20px", fontWeight: 600, color: sparklineColor }}>{loading ? "—" : `${change > 0 ? "+" : ""}${change}%`}</div>
          </div>
        </div>
        ...
        <div style={{ borderTop: "1px solid rgba(255, 107, 44, 0.1)", paddingTop: "16px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {spikes.slice(0, 3).map((spike) => (
              <div key={spike.symbol} style={{ background: "rgba(255, 107, 44, 0.1)", border: "1px solid rgba(255, 107, 44, 0.2)", borderRadius: "var(--r-1)", padding: "6px 10px", fontSize: "12px", color: "var(--data-fg)" }}>
                ${spike.symbol}
              </div>
            ))}
          </div>
