...
export function SentimentGrid() {
  ...
  const getSentimentColor = (avgScore: number): { bg: string; text: string } => {
    if (avgScore > 5) return { bg: "rgba(76, 175, 80, 0.1)", text: "var(--pos)" };
    if (avgScore < 5) return { bg: "rgba(244, 67, 54, 0.1)", text: "var(--neg)" };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px" }}>
        {sentiment.map((entry) => {
          ...
          return (
            <div
              key={entry.symbol}
              style={{ background: colors.bg, border: `1px solid ${colors.text}33`, borderRadius: "var(--r-2)", padding: "16px", opacity: loading ? 0.5 : 1, transition: "opacity 200ms" }}
            >
              ...
              <div style={{ fontSize: "12px", color: "var(--fg-3)" }}>{entry.tweetCount} tweets</div>
            </div>
          );
        })}
      </div>
