// TokenBuzz · detail.jsx — token detail pane
const { Icon, Button, Eyebrow, Ticker, Pill, BuzzDot, Sparkline, Delta, Avatar } = window.TB;
const { fmtPrice, fmtCount } = window.TBWatchlist;

// Mock mentions (X posts)
const MENTIONS = [
...
const SentimentMeter = ({ score, width = 180 }) => {
  ...
  const fillEndA = -Math.PI + ((angle + 90) / 180) * Math.PI; // sweep based on score
  const polar = (ang, rad) => [cx + Math.cos(ang) * rad, cy + Math.sin(ang) * rad];
  ...
  return (
    <svg viewBox={`0 0 ${width} 100`} width={width} style={{ display: "block" }}>
      <path d={`M${sx} ${sy} A${r} ${r} 0 0 1 ${bx} ${by}`} fill="none" stroke="var(--ink-200)" strokeWidth="10" strokeLinecap="round" />
...
const PriceChart = ({ token }) => {
  // Generate richer chart from spark, smoothed
  ...
  const color = token.d24 >= 0 ? "var(--pos)" : "var(--neg)";
        ...
        <div style={{ display: "inline-flex", gap: 8 }}>
          {["1H","4H","24H","7D","30D"].map((w, i) => (
            <span key={w} style={{
              ...
              background: i === 2 ? "rgba(255,179,71,0.15)" : "transparent",
              color: i === 2 ? "#FFB347" : "#A39378",
              cursor: "pointer",
            }}>{w}</span>
          ))}
        </div>
      ...
      <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={200} preserveAspectRatio="none">
        <defs>
          <linearGradient id="cf" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={token.d24 >= 0 ? "#7BC47F" : "#E0664E"} stopOpacity="0.28" />
            <stop offset="100%" stopColor={token.d24 >= 0 ? "#7BC47F" : "#E0664E"} stopOpacity="0" />
          </linearGradient>
        </defs>
        {[0.25, 0.5, 0.75].map(f => (
        ...
        <path d={path} stroke={token.d24 >= 0 ? "#7BC47F" : "#E0664E"} strokeWidth="1.8" fill="none" strokeLinejoin="round" />
      </svg>
    </div>
  );
};
...
const TokenDetail = ({ token, onClose, onAskHum }) => {
  if (!token) return null;
  // Pseudo sentiment score
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--bg)" }}>
      ...
      <div style={{
        ...
        <div style={{
          width: 44, height: 44, borderRadius: 8, background: "var(--inv-bg)",
          color: "var(--inv-fg)", display: "grid", placeItems: "center",
          fontFamily: "var(--font-mono)", fontWeight: 700, fontSize: 16,
        }}>{token.sym.slice(0,2)}</div>
        <div>
      ...
      <div style={{
        ...
        {[
          { l: "Price", v: fmtPrice(token.price), s: <Delta value={token.d24} style={{ fontSize: 13 }} /> },
          ...
          { l: "Sentiment", v: <span style={{ color: score > 20 ? "var(--pos)" : score < -20 ? "var(--neg)" : "var(--neu)" }}>{score > 0 ? "+" : ""}{score}</span>,
      ...
      {/* Chart + sentiment */}
      <div style={{ padding: 20, display: "grid", gridTemplateColumns: "1fr 240px", gap: 16 }}>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: 16, display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <div style={{ textAlign: "center", font: "600 18px var(--font-mono)", color: score > 20 ? "var(--pos)" : score < -20 ? "var(--neg)" : "var(--neu)" }}>
      ...
      <div style={{ padding: "0 20px 20px", flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{
          ...
          overflow: "hidden", flex: 1, minHeight: 0, overflowY: "auto",
        }}>
