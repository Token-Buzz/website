// TokenBuzz · dashboard.jsx — the "Today" landing view.
// The user's first read of the day: macro pulse, biggest spikes,
// Hum's brief, sentiment grid, emerging narratives, alerts.
const { Icon, Button, Eyebrow, Ticker, Pill, BuzzDot, Sparkline, Delta, Avatar } = window.TB;

// ───────────────────────────────────────────────────────────
// Sample data — hand-tuned to look real, deterministic.
// ───────────────────────────────────────────────────────────
const PULSE_SERIES = [
...
const SPIKES = [
  { sym: "MOG",   name: "Mog Coin",   dbuzz: 4180, mentions: 6700,  sent: "bull",
    spark: [3,3,4,4,5,7,8,11,13,15,18,20,23,28], live: true,
    summary: "Three previously dormant whales just retweeted into MOG within 14 minutes." },
  { sym: "PEPE",  name: "Pepe",       dbuzz: 412,  mentions: 48900, sent: "bull",
    spark: [3,4,4,5,4,6,7,7,8,9,11,12,14,18,22], live: true,
    summary: "Accumulation talk from six mid-tier handles. Coinbase Pro volume cleanest since May." },
  { sym: "TURBO", name: "Turbo",      dbuzz: 96,   mentions: 3100,  sent: "bull",
    spark: [10,11,11,12,12,11,12,13,13,14,14,15,16,17,18], live: true,
    summary: "Discord chatter spike from three trader cohorts. Quiet on X — yet." },
  { sym: "WIF",   name: "dogwifhat",  dbuzz: 84,   mentions: 9800,  sent: "bull",
    spark: [8,8,9,9,10,9,10,11,10,12,12,13,14,15,16], live: false,
    summary: "Steady climb. No single catalyst — just broad-based mention growth." },
];
...
const NARRATIVES = [
  { title: "AI agents are back",     mentions: 4820, growth: 312,
    tokens: ["FET", "AGIX", "TAO", "VIRTUAL"],
    handles: 42,
    summary: "Three macro accounts pivoted to AI agent talk after the Anthropic release. Token chatter following." },
  { title: "L2 fees discourse round 9", mentions: 3140, growth: 86,
    tokens: ["ARB", "OP", "BASE"],
    handles: 28,
    summary: "Sentiment turning bearish on L2s. Watch for capital rotation back to L1." },
  { title: "Memecoin rotation stalling", mentions: 8900, growth: -22,
    tokens: ["PEPE", "WIF", "BONK"],
    handles: 64,
    summary: "Big handles quiet. Only PEPE and MOG actively accumulating mindshare." },
  { title: "Restaking exhaustion",   mentions: 2240, growth: -14,
    tokens: ["EIGEN", "ETHFI"],
    handles: 19,
    summary: "Narrative cooling. Mentions down across the cohort. May be opportunity if you're contrarian." },
];
...
const ALERTS = [
  { tone: "buzz", time: "08:42", tag: "BUZZ SPIKE", target: "$MOG",
    body: "Mentions /min jumped from 14 to 218 in the last 30m. Crossed your 10× rule." },
  { tone: "sent", time: "08:18", tag: "SENTIMENT FLIP", target: "$SOL",
    body: "Sentiment crossed −40 for the first time in 12 days." },
  { tone: "handle", time: "07:51", tag: "WHALE HANDLE", target: "@cobie",
    body: "Just posted about $PEPE — first time in 31 days." },
  { tone: "narrative", time: "07:14", tag: "NEW NARRATIVE", target: "AI agents",
    body: "Cluster of 12 handles started co-mentioning $FET / $AGIX / $VIRTUAL inside 90 minutes." },
];

// ───────────────────────────────────────────────────────────
// Atoms specific to dashboard
// ───────────────────────────────────────────────────────────
const fmtCount = (n) => n >= 1_000_000 ? (n/1_000_000).toFixed(1) + "M"
...
const SectionHead = ({ eyebrow, title, meta, action }) => (
  <div style={{ display: "flex", alignItems: "baseline", gap: 14, marginBottom: 12 }}>
    ...
    {title && <span style={{ font: "600 14px var(--font-sans)", color: "var(--fg-1)" }}>{title}</span>}
    {meta && <span style={{ font: "500 11px var(--font-mono)", color: "var(--fg-3)" }}>{meta}</span>}
    <div style={{ flex: 1 }} />
    {action}
  </div>
);
...
const Greeting = () => (
  <div style={{ display: "flex", alignItems: "flex-end", gap: 16, marginBottom: 4 }}>
    <div style={{ flex: 1 }}>
      <Eyebrow style={{ marginBottom: 6 }}>Today · Tue Nov 12 · 09:14 UTC</Eyebrow>
      <div style={{
        font: "600 28px/1.15 var(--font-sans)", letterSpacing: "-0.015em",
        textWrap: "balance",
      }}>
        Morning, Sam.{" "}
...
// 2. KPI strip
// ───────────────────────────────────────────────────────────
const KPI = ({ label, value, delta, deltaFmt = "pct", foot, accent }) => (
  <Card padding={16} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
    ...
    <div style={{ font: "500 11px var(--font-mono)", color: "var(--fg-3)" }}>{foot}</div>
  </Card>
);

const KPIStrip = () => (
  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
    <KPI label="Mentions · 24h" value="2.14M" delta={18.4} foot="across 24 watchlist tokens" />
    <KPI label="Tokens tracked" value="49"   foot="6 watchlists · 412 handles" />
    <KPI label="Alerts fired"   value="14"   delta={6} deltaFmt="raw"
         foot="3 unread · 11 acknowledged" accent="var(--accent)" />
    <KPI label="Net sentiment"  value="+12"  delta={9.2}
         foot="bull bias · widening since 06:00 UTC" accent="var(--pos)" />
  </div>
);

// ───────────────────────────────────────────────────────────
// 3. The Pulse — terminal hero (data-bg, stays dark)
// ───────────────────────────────────────────────────────────
const PulseChart = ({ points, color = "var(--data-pos)", height = 64 }) => {
  const w = 800;
...
const SentimentSplit = ({ bull, neu, bear }) => {
  const total = bull + neu + bear;
  const seg = (v, c) => (
    <div style={{
      width: `${(v/total)*100}%`,
      background: c, height: "100%",
...
const Pulse = () => (
  <div style={{
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14, position: "relative" }}>
      <BuzzDot />
      <span style={{
        font: "600 11px var(--font-sans)", letterSpacing: "0.18em",
        textTransform: "uppercase", color: "var(--data-amber)",
      }}>The buzz pulse · live</span>
      <span style={{ font: "500 11px var(--font-mono)", color: "var(--data-dim)" }}>
        rolling 60min · 412 handles · all watchlists
      </span>
      <div style={{ flex: 1 }} />
    ...
    <div style={{
      display: "grid", gridTemplateColumns: "260px 1fr", gap: 28, position: "relative",
    }}>
      {/* Left: big mentions/min */}
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div>
          <div style={{ font: "500 10px var(--font-sans)", letterSpacing: "0.18em",
            textTransform: "uppercase", color: "var(--data-dim)", marginBottom: 6 }}>Mentions / min</div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            <span style={{
              font: "600 56px/1 var(--font-mono)", fontVariantNumeric: "tabular-nums",
              letterSpacing: "-0.03em", color: "var(--data-fg)",
            }}>2,140</span>
            <span style={{ font: "600 14px var(--font-mono)", color: "var(--data-pos)" }}>+412%</span>
          </div>
          <div style={{ font: "500 11px var(--font-mono)", color: "var(--data-dim)", marginTop: 4 }}>
            vs 60min avg · 418/min
          </div>
        </div>
        <div>
          <div style={{ font: "500 10px var(--font-sans)", letterSpacing: "0.18em",
            textTransform: "uppercase", color: "var(--data-dim)", marginBottom: 8 }}>Sentiment split</div>
          <SentimentSplit bull={62} neu={20} bear={18} />
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6,
            font: "500 10px var(--font-mono)", color: "var(--data-dim)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 0 }}>
        <div style={{ flex: 1, minHeight: 0 }}>
          <PulseChart points={PULSE_SERIES} color="var(--data-pos)" height={120} />
        ...
        <div style={{
          ...
          <span style={{
            font: "600 10px var(--font-sans)", letterSpacing: "0.18em",
            textTransform: "uppercase", color: "var(--data-amber)",
          }}>Loudest right now</span>
          ...
          <div style={{ flex: 1 }} />
          <span style={{ font: "500 11px var(--font-mono)", color: "var(--data-dim)" }}>
            74% of all mentions in last 5min
          </span>
        </div>
      </div>
    </div>
...
// ───────────────────────────────────────────────────────────
// 4. Buzz spikes — 4 token cards
// ───────────────────────────────────────────────────────────
const SpikeCard = ({ t, onClick }) => (
  <Card padding={16} style={{
    ...
    <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
      <span style={{
        font: "600 24px/1 var(--font-mono)", fontVariantNumeric: "tabular-nums",
...
const Spikes = () => (
  <section>
    <SectionHead
      eyebrow="Biggest spikes · last hour"
      meta="ranked by mention rate Δ"
      action={<Button variant="quiet" size="sm">See all movers →</Button>}
    />
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
...
// ───────────────────────────────────────────────────────────
// 5. Hum's morning brief
// ───────────────────────────────────────────────────────────
const Brief = () => (
  <Card padding={20} style={{
    display: "grid", gridTemplateColumns: "60px 1fr 240px", gap: 24,
    ...
    <div style={{ minWidth: 0 }}>
      ...
      <div style={{
        font: "500 16px/1.5 var(--font-sans)", letterSpacing: "-0.005em",
        textWrap: "pretty", color: "var(--fg-1)",
      }}>
        ...
        $PEPE accumulation chatter is the loudest since May, but @degenspartan is loudly bearish, which has historically been a contrarian buy.{" "}
    ...
    <div style={{
      ...
      paddingLeft: 20, alignSelf: "stretch",
    }}>
      <Eyebrow>Quick asks</Eyebrow>
      {[
        "What's driving $MOG?",
        "Is the $PEPE bearish thread credible?",
        "Cluster the AI-agent handles.",
      ].map(q => (
        <button key={q} style={{
...
// ───────────────────────────────────────────────────────────
// 6. Sentiment grid (heatmap-ish)
// ───────────────────────────────────────────────────────────
const SentCell = ({ t }) => {
  const score = t.score;
  // Map -100..+100 to color intensity
  ...
  return (
    <div style={{
      ...
      <div style={{
        font: "600 18px/1 var(--font-mono)", fontVariantNumeric: "tabular-nums",
      ...
      action={
        <div style={{ display: "inline-flex", gap: 12, font: "500 10px var(--font-mono)", color: "var(--fg-3)" }}>
          <span><span style={{
            ...
            background: "var(--data-pos)", marginRight: 4, opacity: 0.5,
          }} />bull</span>
          <span><span style={{
            display: "inline-block", width: 8, height: 8, borderRadius: 2,
            background: "var(--data-amber)", marginRight: 4, opacity: 0.5,
          }} />mixed</span>
          <span><span style={{
            display: "inline-block", width: 8, height: 8, borderRadius: 2,
            background: "var(--data-neg)", marginRight: 4, opacity: 0.5,
          }} />bear</span>
        </div>
      }
    />
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
      {SENTIMENT_TOKENS.map(t => <SentCell key={t.sym} t={t} />)}
    </div>
  </Card>
);
...
const NarrativeRow = ({ n, isLast }) => (
  <div style={{
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <span style={{
        font: "600 14px var(--font-sans)", letterSpacing: "-0.01em", flex: 1, minWidth: 0,
      }}>{n.title}</span>
      <Delta value={n.growth} style={{ fontSize: 12 }} />
    </div>
    <div style={{
      font: "400 12px/1.5 var(--font-sans)", color: "var(--fg-2)", textWrap: "pretty",
    }}>{n.summary}</div>
    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
      {n.tokens.map(s => <Ticker key={s} symbol={s} size="sm" />)}
      <div style={{ flex: 1 }} />
      <span style={{ font: "500 11px var(--font-mono)", color: "var(--fg-3)" }}>
        {fmtCount(n.mentions)} · {n.handles} handles
      </span>
    </div>
  </div>
);
...
const Narratives = () => (
  <Card padding={18} style={{ display: "flex", flexDirection: "column" }}>
    <SectionHead
      ...
      meta="Hum clustered · last 12h"
      action={<Button variant="quiet" size="sm">All narratives →</Button>}
    />
    <div>
      {NARRATIVES.map((n, i) => (
...
// 8. Live stream
// ───────────────────────────────────────────────────────────
const StreamItem = ({ m, isLast }) => {
  const sentColor = m.sent === "bull" ? "var(--pos)"
                  : m.sent === "bear" ? "var(--neg)"
                  : "var(--neu)";
  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ font: "400 12px/1.5 var(--font-sans)", color: "var(--fg-1)", textWrap: "pretty" }}>
          {m.text}
        </div>
      </div>
      <span style={{
        color: sentColor, font: "600 12px var(--font-mono)",
        paddingTop: 4,
      }}>{sentGlyph}</span>
    </div>
  );
    ...
    <SectionHead
      ...
      action={
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
          <BuzzDot />
          <span style={{ font: "500 11px var(--font-mono)", color: "var(--fg-3)" }}>updated 4s ago</span>
        </div>
      }
    />
    <div>
      {STREAM.map((m, i) => (
        <StreamItem key={i} m={m} isLast={i === STREAM.length - 1} />
      ))}
    </div>
  </Card>
);
...
// 9. Alerts log
// ───────────────────────────────────────────────────────────
const AlertRow = ({ a, isLast }) => {
  const tones = {
    buzz:      { glyph: "●",  color: "var(--accent)" },
    sent:      { glyph: "◆",  color: "var(--neu)"    },
    handle:    { glyph: "@",  color: "var(--data-violet)" },
    narrative: { glyph: "✶",  color: "var(--data-teal)"   },
  };
  const tone = tones[a.tone] || tones.buzz;
  return (
    <div style={{
      display: "grid", gridTemplateColumns: "18px 56px 1fr", gap: 10,
      padding: "10px 0", alignItems: "flex-start",
      borderBottom: isLast ? "none" : "1px solid var(--border-hairline)",
    }}>
      <span style={{
        color: tone.color, font: "600 14px var(--font-mono)",
        textAlign: "center",
      }}>{tone.glyph}</span>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2, flexWrap: "wrap" }}>
          <span style={{
            font: "600 10px var(--font-sans)", letterSpacing: "0.15em",
            textTransform: "uppercase", color: tone.color,
          }}>{a.tag}</span>
          <span style={{
            font: "600 12px var(--font-mono)", color: "var(--fg-1)",
          }}>{a.target}</span>
        </div>
...
const AlertsLog = () => (
  <Card padding={18} style={{ display: "flex", flexDirection: "column" }}>
    <SectionHead
      eyebrow="Alerts fired today"
      meta="4 unread"
      action={<Button variant="quiet" size="sm">Rules →</Button>}
    />
    <div>
      {ALERTS.map((a, i) => (
        <AlertRow key={i} a={a} isLast={i === ALERTS.length - 1} />
      ))}
    </div>
  </Card>
);
...
const Dashboard = () => (
  <div style={{
    ...
    <div style={{
      ...
      font: "500 11px var(--font-mono)", color: "var(--fg-4)",
      letterSpacing: "0.04em",
    }}>
      TokenBuzz · social intelligence, tuned to the human ear · v0.9.4
    </div>
  </div>
);

window.TBDashboard = { Dashboard };
