...
- `playwright` and `@clerk/testing` are now committed root devDependencies, and a committed marketing E2E suite exists (`npm run test:e2e`, config `playwright.config.ts`, tests in `e2e/`) that runs in CI against the offline marketing app. There is now ALSO a committed **authed application E2E suite** (`npm run test:e2e:application`, config `playwright.application.config.ts`, tests in `e2e/application/`) that drives the Clerk-authed app fully offline — a Playwright `globalSetup` boots dynalite and recreates the `infra/db.ts` tables/GSIs, `next dev` (:3002) is pointed at it via env, and sign-in is headless via `@clerk/testing/playwright` (Clerk test tokens + the fixed dev OTP `424242` for `+clerk_test` addresses). It is **opt-in in CI**: it runs only on `master` pushes whose head commit message contains the literal tag `[E2E]`, never on PRs, and when it runs it gates (blocks) the production deploy. The ad-hoc browser-testing recipe described above remains useful for exploratory/local checks of the authed application app (Clerk + DynamoDB): install the browser ad hoc (`npx playwright install chromium`) and don't commit the throwaway harness scripts.
...
- `CLERK_TEST_PUBLISHABLE_KEY` — a `pk_test…` value (mapped to env `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`). Only needed for the opt-in `[E2E]` authed application E2E job; must be the Clerk **dev/test** instance, not production.
- `CLERK_TEST_SECRET_KEY` — a `sk_test…` value (mapped to env `CLERK_SECRET_KEY`). Only needed for the opt-in `[E2E]` authed application E2E job; must be the Clerk **dev/test** instance.
- `CLERK_TEST_EMAIL` — a `+clerk_test@…` address used for headless sign-in. Only needed for the opt-in `[E2E]` authed application E2E job; must be the Clerk **dev/test** instance.
